...
func (w *Web) allFailures(rw http.ResponseWriter, req *http.Request, _ *utils.State, isAdmin bool) {
	// Parse input
	q := req.URL.Query()
	...
	err = w.engine.Where("service = ?", serviceId).Asc("created_at").Find(&failures)
	if err != nil {
		log.Println(err)
		http.Error(rw, "Failed to read from database", http.StatusInternalServerError)
		return
